...
final class DetectionResultsModelsTests: XCTestCase {
    ...
    func testModelEncodingPerformance() throws {
        // Given
        let detectedTexts = Array(1...50).map { idx in
            DetectedText(text: "Text \(idx)", boundingBox: CGRect.zero, confidence: 0.9, characterBoxes: [])
        }
        let storedResult = StoredOCRResult(detectedTexts: detectedTexts, image: testImage)
        let encoder = JSONEncoder()
    ...
    private func removingJSONKey(_ key: String, from encodedData: Data) throws -> Data {
    ...
    private func replacingJSONValue(_ value: Any, forKey key: String, in encodedData: Data) throws -> Data {
        guard var jsonObject = try JSONSerialization.jsonObject(with: encodedData) as? [String: Any] else {
            XCTFail("Expected top-level JSON dictionary")
            return encodedData
        }
